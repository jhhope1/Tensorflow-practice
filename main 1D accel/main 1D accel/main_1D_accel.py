...
Destination=tf.placeholder(tf.float32, [None, 1])
Vel=tf.placeholder(tf.float32, [None, 1])#로봇의 속도'''
Rx=tf.placeholder(tf.float32, [None,1])#갱신된 위치'''
...
W1=tf.Variable(tf.random_normal([3, 10], -0.01 , 0.01))
...
for step in range(100):
    ...
    if(step+1)%10==0:
        ...
        'cost: %.3f'%sess.run(cost, feed_dict={Vel:v_data, Rx:x_data, Destination:des_data}))
x_test=np.array(
